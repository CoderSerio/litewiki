...
import {
  DEFAULT_PROFILE,
  type PromptProfile,
} from "../../src/cli/commands/profile/constant.js";
import {
  listProfiles,
  writeProfileFile,
...
test("listProfiles returns builtin default plus saved profiles", async (t) => {
  const tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "litewiki-profiles-"),
  );
...
test("profile files override builtin defaults when ids match", async (t) => {
  const tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "litewiki-profiles-override-"),
  );
  const profilesDir = path.join(tmpRoot, "profiles");
  await fs.mkdir(profilesDir, { recursive: true });
  t.after(async () => {
  ...
  assert(
    overridden?.filePath,
    "expected filePath to be set for overridden builtin",
  );
